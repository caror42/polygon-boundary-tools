...
def add_circles(num, polyg, grain = 5)
  #setting main parameters for continued reference
  minx = polyg.map { |x| x[0] }.min
  maxx = polyg.map { |x| x[0] }.max
  width = maxx - minx
  miny = polyg.map { |x| x[1] }.min
  maxy = polyg.map { |x| x[1] }.max
  height = maxy - miny
  ...
  covered = NArray.int(width / grain, height / grain).fill(0)
  ...
  while count < num
    ...
    (1...covered.shape[0]).each do |k|
      (1...covered.shape[1]).each do |l|
        #if the point is outside the polygon, go next
        next if covered[k, l] == -1
        new_center = [k, l]

        #max radius if all points happen to be inside
        #setting bounds for iteration
        max_radius = [k, l, covered.shape[0] - k, covered.shape[1] - l].min
        
        #if even the max radius has no chance to beat high score, continue
        next if max_radius < Math.sqrt(max_points / 4) #4 for wiggle room
        ...
        (0...max_radius).each do |r|
          ...
          works = true
          points_covered = 0
          ((k - r - 1)..(k + r + 1)).each do |m|
            ((l - r - 1)..(l + r + 1)).each do |n|
              if Math.sqrt((m - k)**2 + (n - l)**2) <= r
                if covered[m, n] == -1
          ...
          if works && points_covered > curr_covered
            found_circ = true
            curr_rad = r
          ...
          #redundancy
          if found_circ && curr_covered > max_points
            top_radius = curr_rad
    ...
    ((top_center[0] - top_radius - 1)..(top_center[0] + top_radius + 1)).each do |m|
      ((top_center[1] - top_radius - 1)..(top_center[1] + top_radius + 1)).each do |n|
        if Math.sqrt((m - top_center[0])**2 + (n - top_center[1])**2) < top_radius
          covered[m, n] = 1
        end
      end
    end
    ...
    # print(covered.size)
    # if(count == num)
    #    covered.each_with_indices do |val, i, j|
    #       if val == 1
    #         cov_county += 1
    #       elsif val == -1
    #         out_range += 1
